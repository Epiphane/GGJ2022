...
export class CityBuilderState extends State {
  ...
  constructor() {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      unit.width = 50;
      unit.height = 50;
      unit.position = this.game.size
        .copy()
        .mult(Math.random() - 0.5, Math.random() - 0.5)
        .mult(1 / 3);
    ...
    for (let i = 0; i < 10; i++) {
      ...
      unit.position = this.game.size
        .copy()
        .mult(Math.random() * 0.9, Math.random() * 0.9);
  ...
  dragend_0(pos: Point) {
    if (this.dragStartPoint) {
      ...
      this.units.forEach((selectable) => {
        ...
        const hovering =
          unit.position.x - unit.width / 2 >= minX &&
